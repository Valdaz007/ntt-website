                <!-- Broadband Internet -->
                <link rel="stylesheet" type="text/css" href="static/css/css-templates/brod-int.css">
                <!-- Direct Optical Fiber Plans Section -->
                <section class="row dof" id="internet">
                    <h2>DIRECT OPTICAL FIBER<span class="dof-icon"><img src="./static/img/dof-icon.png"></span></h2>
                    <hr>
                    <h4>UNLIMITED FULL DUPLEX</h4>
                    <div class="row g-3 dof-plans">
                        <div class="col-6 col-sm-4">
                            <div class="dof-plan dof-10"></div>
                        </div>
                        
                        <div class="col-6 col-sm-4">
                            <div class="dof-plan dof-20"></div>
                        </div>

                        <div class="col-6 col-sm-4">
                            <div class="dof-plan dof-50"></div>
                        </div>

                        <div class="col-6 col-sm-12">
                            <h5>
                                One off installation cost will be applied and varies depending on customer
                                location from the main fiber network.
                            </h5>
                        </div>
                    </div>
                </section><br><br>

                <!-- NEXT Broadband Service Plans Section -->
                <section class="next">
                    <h2>NEXT BROADBAND SERVICE<span><img src="./static/img/pon-icon.png"></span></h2>
                    <h4>FAST & RELIABLE FIBER TO THE HOME GPON SERVICE</h4>
                    <hr>
                    <h5>UNLIMITED FULL DUPLEX PLANS</h5>
                    <br>
                    <div class="row g-3 d-plans">
                        <div class="col-6 col-md-4"><div class="gpon"></div></div>
                        <div class="col-6 col-md-4"><div class="gpon"></div></div>
                        <div class="col-6 col-md-4"><div class="gpon"></div></div>
                        <div class="col-6 col-md-4"><div class="gpon"></div></div>
                        <div class="col-6 col-md-4"><div class="gpon"></div></div>
                        <div class="col-6 col-md-4"><div class="gpon"></div></div>
                    </div>
                    <br>
                    <h5>
                        One off installation cost will be applied and varies depending on customer
                        location from the main fiber network.
                    </h5>
                </section><br><br>

                <!-- VSAT Broadband Service Plans Section -->
                <section class="row vsat">
                    <h2>VSAT INTERNET SERVICE<span><img src="./static/img/vsat-icon.png"></span></h2>
                    <hr>
                    <img class="img-fluid" src="../static/img/data-plans/vsat-data-plans.png">
                    <h5>
                        One Off VSAT Electronic & Terminal Cost K4,150.00.<br>
                        Installation Cost will depend on Location and Setup.
                    </h5>
                </section><br><br>