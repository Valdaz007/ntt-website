<?php
    $title = "Cloud Services | Network Infrastructure | Telco Procurement | SOLUTIONS";
    $desc = "Our Services ranges from Application, Website & Email Hosting, Network Cabling, Setup & Configuration and Telco Procurement Service.";
    $stylesheet = "solutions.css";
    require_once "includes/header.php";
?>

        <main>
            <section class="hero">
                <img src="./static/img/hero-img/soluntions-hero02.png" alt="">
        
                <div class="hero-content">
                    <div class="hero-content-inner">
                        <h2 class="title">THE BEST IT SOLUTION FOR YOUR BUSINESS!</h2>
                        <h3 class="sub-title">CONTACT US TODAY FOR A CONSULTATION</h3>
                        <a class="btn-sm btn-primary" type="button" href="./contacts">CONTACT US NOW</a>
                    </div>
                </div>
            </section><br>

            <div class="content container-fluid">
                <div class="row">
                    <div class="col-lg-9">
                        <!-- CLOUD HOSTING -->
                        <section class="row g-3 services mb-5" id="cloud">
                            <h2>CLOUD HOSTING<span class="con-icon"><img src="./static/img/cloud.png"  class="img-fluid"><span></h2>
                            <hr>
                            
                            <div class="col-12 col-md-4 micard-wrapper">
                                <div class="micard mc1-1">
                                    <div class="micard-img"></div>
                                    <div class="micard-body p-2">
                                        <h4>Website & Email</h4>
                                        <p>
                                            Attract potential customers by looking more professional with a company 
                                            branded email (example@companyname.com or example@companyname.com.pg) and 
                                            expand your company’s influence online with a company website showcasing 
                                            your products and services.
                                        </p>
                                    </div>
                                </div>
                            </div>
                            
                            <div class="col-12 col-md-4 micard-wrapper">
                                <div class="micard  mc1-2">
                                    <div class="micard-img"></div>
                                    <div class="micard-body p-2">
                                        <h4>Application</h4>
                                        <p>
                                            Improve your business performance and productivity by running your business 
                                            application on the cloud. With our help your business application that’s running 
                                            on your premise with many risks such as data loss, downtime and hardware faults 
                                            can be mitigated by hosting the application on a cloud.
                                        </p>
                                    </div>
                                </div>
                            </div>
                            
                            <div class="col-12 col-md-4 micard-wrapper">
                                <div class="micard mc1-3">
                                    <div class="micard-img"></div>
                                    <div class="micard-body p-2">
                                        <h4>Virtualization</h4>
                                        <p>
                                            We aim to be at the forefront of technological innovation, enabling organizations to 
                                            navigate the digital age seamlessly.
                                            We offer advanced server virtualization solutions that enable businesses to consolidate 
                                            their physical servers, improve resource utilization, and enhance server management.
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </section>

                        <!-- NETWORK INFRASTRUCTURE -->
                        <section class="row g-3 services mb-5" id="netw">
                            <h2>NETWORK INFRASTRUCTURE<span class="con-icon"><img src="./static/img/net-infras-icon2.png"><span></h2>
                            <hr>

                            <div class="col-12 col-md-6">
                                <div class="micard mc2-1">
                                    <div class="micard-img"></div>
                                    <div class="micard-body p-2">
                                        <h4>WiFi Deployment</h4>
                                        <p>
                                            Experience seamless connectivity with our cutting-edge WIFI Deployment services! 
                                            Say goodbye to dead zones and slow internet speeds. Whether you need to cover a 
                                            small office space or a large commercial complex, our expert team will design and 
                                            install a robust WIFI network tailored to your specific needs. Enjoy fast, reliable, 
                                            and secure internet access from every corner of your space.
                                        </p>
                                    </div>
                                </div>
                            </div>

                            <div class="col-12 col-md-6">
                                <div class="micard mc2-2">
                                    <div class="micard-img"></div>
                                    <div class="micard-body p-2">
                                        <h4>Structured Cabling</h4>
                                        <p>
                                            Experience enhanced efficiency, improved communication, and reduced downtime with 
                                            our industry-leading cabling solutions. Whether it's for your office, data center, 
                                            or campus, trust us to deliver a reliable and organized network that empowers your 
                                            business to thrive. Elevate your connectivity to new heights with our trusted 
                                            Structured Cabling services today!
                                        </p>
                                    </div>
                                </div>
                            </div>

                            <div class="col-12 col-sm-6">
                                <div class="mc2-3 micard">
                                    <div class="micard-img"></div>
                                    <div class="micard-content p-2">
                                        <h4>Routing & Switching</h4>
                                        <p>
                                            Stay ahead of the competition with a robust and agile network infrastructure that 
                                            ensures seamless data flow, heightened security, and optimal performance. 
                                            Our team of skilled engineers will tailor a customized Routing & Switching setup 
                                            to match your specific requirements, enabling faster data transfer, reduced latency, 
                                            and improved user experience.
                                        </p>
                                    </div>
                                </div>
                            </div>

                            <div class="col-12 col-sm-6">
                                <div class="mc2-4 micard">
                                    <div class="micard-img"></div>
                                    <div class="micard-content p-2">
                                        <h4>Network Management</h4>
                                        <p>
                                            Take control of your network like never before with our comprehensive Network Management 
                                            services! Our expert team will provide you with a powerful suite of tools and technologies 
                                            to efficiently monitor, optimize, and secure your network infrastructure. Gain real-time 
                                            insights into network performance, proactively identify and resolve issues, and ensure peak 
                                            efficiency at all times.
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </section>

                        <!-- PROCUREMENT SERVICES -->
                        <section class="row g-3 services mb-5" id="procure">
                            <h2>PROCUREMENT SERVICE<span class="con-icon"><img src="./static/img/procure-icon2.png"><span></h2>
                            <hr>
                            <div class="col-12 col-md-6">
                                <div class="mc3-1 micard">
                                    <div class="micard-img"></div>
                                    <div class="micard-body p-2">
                                        <h4>Enterprice Network Devices</h4>
                                        <p>
                                            A robust and efficient network is the backbone of any successful enterprise.
                                            Our procurement service for cutting-edge devices is designed to elevate your organization's
                                            connectivity, security, and overall performance.
                                        </p>
                                    </div>
                                </div>
                            </div>
                            
                            <div class="col-12 col-md-6">
                                <div class="mc3-2 micard">
                                    <div class="micard-img"></div>
                                    <div class="micard-body p-2">
                                        <h4>Servers & PC's</h4>
                                        <p>
                                            Servers and PCs plays a pivotal role in driving your organization's success, and we're excited
                                            to be able to help in procuring the required hardwares that will elevate your operations to new 
                                            heights of efficiency
                                        </p>
                                    </div>
                                </div>
                            </div>
                            
                            <div class="col-12 col-md-6">
                                <div class="mc3-3 micard">
                                    <div class="micard-img"></div>
                                    <div class="micard-body p-2">
                                        <h4>Enterprice Software Solutions</h4>
                                        <p>
                                            In today's dynamic business landscape, the right software solutions can be the driving force behind 
                                            growth, efficiency, and innovation. We understand the challenges your organization faces and are 
                                            excited to present a compelling proposal for delivering cutting-edge enterprise software solutions 
                                            that will reshape the way you operate, compete, and succeed.
                                        </p>
                                    </div>
                                </div>
                            </div>
                            
                            <div class="col-12 col-md-6">
                                <div class="mc3-4 micard">
                                    <div class="micard-img"></div>
                                    <div class="micard-body p-2">
                                        <h4>Telecommunication Infrastructure & Solutions</h4>
                                        <p>
                                            At Next Town Technology, we offer a wide array of cutting-edge solutions that cater to the diverse needs 
                                            of modern telecommunication. From designing and deploying robust fiber optic networks that deliver 
                                            lightning-fast data transfer to implementing state-of-the-art radio wave communication systems for seamless 
                                            connectivity in remote areas, our expert team ensures you stay ahead in the digital age.
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </section>
                    </div>
                    
                    <!-- Enquire Form - SIDEBAR -->
                    <aside class="col-12 col-lg-3 side-navbar">
                        <div class="sticky-box">
                            <form action="send.php" method="post" class="enquire-form">
                                <h4>Enquire Now</h4>
                                <hr>
                                <input type ="text" name="page" value="solutions" style="display:none">
                                <input type ="text" name="subject" value="Solutions Enquiry" style="display:none">
                                <input class="form-control soln-input" type="text" name="name" placeholder="Name" required>
                                <input class="form-control soln-input" type="email" name="email" placeholder="Email" required>
                                <input class="form-control soln-input" type="text" name="phone" placeholder="Phone">
                                <textarea class="form-control soln-input" rows="4" type = "text" name="message" placeholder="Message" required></textarea>
                                <input class="btn btn-primary soln-input" type="submit" name="send">
                            </form>
                            <br>
                            <a id="top" type="button" class="btn btn-warning" href="#">Top</a>
                        </div>
                    </aside>
                </div>
            </div>
            <br>
        </main>

<?php require_once "includes/footer.php"; ?>